...
namespace VeritabanıGörsel
{
    public partial class sellProduct : Form
    {
        ...
        private void buttonsClass1_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("Ürün stok 0 satış yapılamaz!", "Uyarı", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
        ...
        private void chanceUserToolStripMenuItem_Click(object sender, EventArgs e)
        {

            this.Hide();
            LoginScreen.loginscreen.ShowDialog();
        }
        ...
        private void buttonsClass4_search_Click(object sender, EventArgs e)
        {
            if (maskedTextBox1.Text != "" && maskedTextBox2.Text != "")
            {
                if (radioButton6.Checked)
                {
                    if (radioButton4.Checked)
                    {
                        sqlquery = "select * from product_tbl where pYear between'" + Convert.ToInt32(maskedTextBox1.Text) + "' and '" + Convert.ToInt32(maskedTextBox2.Text) + "' order by pYear asc";
                        dataGridView1 = database.GridveriİslemDoldur(dataGridView1, sqlquery);
                    }
                    else if (radioButton5.Checked)
                    {
                        sqlquery = "select * from product_tbl where pYear between'" + Convert.ToInt32(maskedTextBox1.Text) + "' and '" + Convert.ToInt32(maskedTextBox2.Text) + "' order by pYear desc";
                        dataGridView1 = database.GridveriİslemDoldur(dataGridView1, sqlquery);
                    }
                }
            }
